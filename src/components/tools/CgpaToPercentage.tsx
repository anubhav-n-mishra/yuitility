...
export default function CgpaToPercentage({
  ...
  if (percentage !== null) {
    if (percentage >= 75) {
      ...
      badgeColor = 'text-emerald-600 dark:text-emerald-400 border-emerald-500/30 bg-emerald-950/40';
    } else if (percentage >= 60) {
      division = 'First Division / First Class';
      badgeColor = 'text-indigo-600 dark:text-indigo-400 border-indigo-500/30 bg-indigo-950/40';
      ...
      badgeColor = 'text-rose-600 dark:text-rose-400 border-rose-500/30 bg-rose-950/40';
    }
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left Inputs */}
        <div className="lg:col-span-2 bg-white dark:bg-zinc-900/60 shadow-sm p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 space-y-6">
          <div>
            <div className="flex items-center justify-between mb-2">
              <label htmlFor="cgpa-input" className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">
                Enter Your CGPA (Cumulative Grade Point Average)
              </label>
              <span className="text-xs text-zinc-500 dark:text-zinc-400 font-mono">Scale 0.0 – 10.0</span>
            </div>
            <div className="relative">
              <input
                ...
                className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl px-4 py-3.5 text-3xl font-bold font-mono text-white focus:outline-none focus:border-indigo-500 transition"
              />
              <span className="absolute right-4 top-4 text-xs font-semibold px-2.5 py-1 rounded-lg bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-900 dark:text-zinc-100 font-semibold border border-zinc-200 dark:border-zinc-700">
                / 10.0 Max
              </span>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <label className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 flex items-center gap-1.5">
                <School className="w-4 h-4 text-indigo-600 dark:text-indigo-400" /> Select University or Board Formula
              </label>
              <span className="text-xs text-indigo-600 dark:text-indigo-400 font-medium">10 Official Rules</span>
            </div>
                ...
                <button
                  ...
                  className={`p-3 text-left rounded-xl border transition-all ${
                    formula === univ.id
                      ...
                      : 'border-zinc-200 dark:border-zinc-800/90 bg-zinc-50 dark:bg-zinc-950/50 text-zinc-500 dark:text-zinc-400 hover:text-zinc-800 dark:text-zinc-200 hover:border-zinc-200 dark:border-zinc-700'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <div className="font-semibold text-xs text-zinc-900 dark:text-zinc-100">{univ.name}</div>
                  ...
                  <div className="text-[11px] text-zinc-500 dark:text-zinc-400 mt-1 line-clamp-1">{univ.subtitle}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          {formula === 'custom' && (
            <div className="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 space-y-2">
              <label className="block text-xs font-semibold text-zinc-700 dark:text-zinc-300">Custom Multiplier Factor</label>
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl px-3 py-2 text-white font-mono text-sm w-36 focus:outline-none focus:border-indigo-500"
                />
                <span className="text-xs text-zinc-500 dark:text-zinc-400">Formula: CGPA × {customMultiplier}</span>
              </div>
            </div>
          )}
        </div>
        ...
        {/* Right Output Card */}
        <div className="bg-white dark:bg-zinc-900/60 shadow-sm p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Equivalent Percentage</span>
              <Award className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
            </div>

            <div className="text-5xl sm:text-6xl font-black font-mono text-indigo-600 dark:text-indigo-400 tracking-tight my-2">
              {percentage !== null ? `${percentage.toFixed(2)}%` : '—'}
            </div>

            {formulaDisplay && (
              <div className="text-xs text-zinc-700 dark:text-zinc-300 font-mono bg-zinc-50 dark:bg-zinc-950/90 p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 my-3">
                <div className="text-[10px] text-zinc-500 dark:text-zinc-500 uppercase tracking-widest mb-0.5">Applied Formula</div>
            ...
            {division && (
              <div className="mt-2 space-y-2">
                <div className="text-[11px] text-zinc-500 dark:text-zinc-400 font-medium">Academic Classification:</div>
            ...
            <div className="mt-4 pt-3 border-t border-zinc-200 dark:border-zinc-800/80 text-[11px] text-zinc-500 dark:text-zinc-400">
              <span className="font-semibold text-zinc-700 dark:text-zinc-300">Official Standard: </span>
      ...
      <div className="bg-white dark:bg-zinc-900/60 shadow-sm p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div>
            <h3 className="text-base font-display font-bold text-zinc-900 dark:text-white flex items-center gap-2">
              <GraduationCap className="w-5 h-5 text-indigo-600 dark:text-indigo-400" /> Official Indian Universities &amp; Boards Conversion Rules
            </h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-zinc-50 dark:bg-zinc-950 text-zinc-500 dark:text-zinc-400 border-b border-zinc-200 dark:border-zinc-800 uppercase font-mono">
              <tr>
            ...
            <tbody className="divide-y divide-slate-800/60 font-medium text-zinc-700 dark:text-zinc-300">
              {UNIVERSITIES.filter((u) => u.id !== 'custom').map((u) => (
                <tr key={u.id} className="hover:bg-zinc-50 dark:bg-zinc-950/40 transition-colors">
                  <td className="py-3 px-4 font-semibold text-white">{u.name}</td>
                  <td className="py-3 px-4 font-mono text-zinc-500 dark:text-zinc-400">10.0</td>
                  <td className="py-3 px-4 font-mono text-indigo-300 font-bold">{u.formulaLabel}</td>
                  <td className="py-3 px-4 text-zinc-500 dark:text-zinc-400">{u.circularRef}</td>
                  <td className="py-3 px-4 text-right">
                    <button
                      type="button"
                      onClick={() => setFormula(u.id)}
                      className="inline-flex items-center gap-1 text-[11px] font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-300 bg-indigo-950/60 px-2.5 py-1 rounded-lg border border-indigo-800/40 hover:border-indigo-600 transition"
                    >
